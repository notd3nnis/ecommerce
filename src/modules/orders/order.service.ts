...
import { IOrderType, OrderItem, OrderStatus, PaymentStatus } from "../../types/IOrder"
import { ICartType } from "../../types/ICart";
...
export const checkOut = async (orderBody: IOrderType, cartId: string) => {
    ...
    if (duplicateOrder) {
        throw new ApiError(httpStatus.BAD_REQUEST, "An order with this cart has already been processed");
    }

    const cart = await Cart.findById({_id: cartId, userId: orderBody.userId}).populate("items.productId");
    ...
    for (const item of cart.items) {
        const product = await Product.findById(item.productId as string) as getIproduct; 

        if (!product) {
            throw new ApiError(httpStatus.NOT_FOUND, `An item in your cart no longer exists.`);
        }

        if (item.quantity > product.stock) {
            throw new ApiError(httpStatus.BAD_REQUEST, `Insufficient stock for ${product.name}. Only ${product.stock} left in stock.`);
        }
...
export const cancelOrder = async (orderId: IOrderType) => {
    ...
    if (['shipped', 'delivered', 'cancelled'].includes(order.status)) {
      throw new ApiError(httpStatus.BAD_REQUEST, `Cannot cancel an order that is already ${order.status}.`);
    }
